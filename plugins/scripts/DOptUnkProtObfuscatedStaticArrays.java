...
/**
 * Sample dexdec IR optimizer plugin. This plugin makes us of the emulator (IDState), and requires JEB 4.6+.<br>
 * Attempt to clean obfsucated code making use of static array values computed during class initialization, such as:<br>
 * 
 * <pre>
 *  static {
 ...
 * </pre>
 * 
 * This optimizer is unsafe and custom, it should not be globally enabled, as it is assumed that all detected
 ...
 *
 */
public class DOptUnkProtObfuscatedStaticArrays extends AbstractDOptimizer {
    int replcnt;
    int triggercnt;
    Set<String> fieldCsigs = new LinkedHashSet<>();

    public DOptUnkProtObfuscatedStaticArrays() {

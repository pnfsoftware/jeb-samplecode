...
"""
DEPRECATION WARNING:
Inefficient implementation as of JEB 3.17.
For greater performance, use the DexDecompilerExporter class to export source code generated by an IDexDecompilerUnit.
(See sample file DecompileFile.py for an example on how to use DexDecompilerExporter)
Implementation still okay for native code decompilers.
...
class Decomp(Runnable):
  ...
  def run(self):
    raise Exception('DEPRECATED: see doc string - uncomment this line if you really want to use this script')
